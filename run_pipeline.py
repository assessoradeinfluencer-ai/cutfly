...
def main():
	...
	try:
		...
		if not skip_extract:
			# Run extraction with extract_scenes.py
			cmd = [
				python,
			...
			if video_path:
				analysis_file = analysis_output_path(video_path, output_dir)
				cmd.extend(["--analysis", str(analysis_file)])
			else:
				cmd.extend(["--analysis-dir", str(output_dir)])
		...
		if not skip_export:
			...
			if video_path:
				...
				cmd.extend(["--analysis", str(output_dir)])
			if use_rendered:
		...
		if is_linux and resolve_cfg.get("auto_start"):

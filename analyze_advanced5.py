...
if torch.cuda.is_available():
    ...
    print(f"\n💎 GPU CUDA: {GPU_NAME} ({GPU_MEMORY:.1f}GB)")
    print(f"   CUDA: {torch.version.cuda}, PyTorch: {torch.__version__}")
elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
    GPU_DEVICE = "mps"
    print(f"\n💎 GPU MPS detected, using Apple Silicon accelerator")
    print(f"   PyTorch: {torch.__version__}")
else:
    GPU_DEVICE = "cpu"
    print("\n⚠️  No GPU backend detected - using CPU (will be slow)")
...
def fallback_simple_analysis(video_path, output_dir, max_scene_length=40, default_speed=2.0):
    ...
    while start < duration:
        ...
        seg = min(float(max_scene_length), remaining)
        scenes.append({
            "scene_num": scene_num,
            "start_time": start,
            "duration": seg,
        ...
        start += seg
        scene_num += 1
    output_duration = sum(s["duration"] / s["speed"] for s in scenes)
    analysis = {
        ...
        "summary": {
            ...
            "compression_ratio": int((1.0 - (output_duration / duration)) * 100) if duration > 0 else 0,
            "interesting": 0,
            "moderate": len(scenes),
            "low": 0,
            "boring": 0,
    ...
    print(f"✓ Fallback analysis written: {out_path}")
    return out_path
...
def main():
    ...
    if videos_to_process:
        ...
        llava_model = load_llava_model()
        if llava_model is None:
            print("⚠️  LLM model not available. Proceeding with simple fallback analysis (moderate @ 2x).")

    for video_path in videos_to_process:
        try:
            if llava_model is None:
                output_file = fallback_simple_analysis(
                    video_path,
                    output_dir,
